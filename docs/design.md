...
- **What ends a cursor walk of a works list?** → An empty page, not an absent token. Crossref keeps returning a `next-cursor` past the end of a list, so relaying it verbatim would loop a caller forever. `nextWorksCursor` is withheld once a page comes back empty, which keeps "no continuation field means the list is exhausted" true across both paging modes on the same tool.
